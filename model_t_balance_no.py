...
class net(nn.Module):
    ...
    def forward(self, text_a, text_b, t, index, bu, dalei, inv_flag, app_flag, tr):
        ...
        return (pred_t, pred_y_c, pred_y_a, pred_y_a_c)
    ...
    def lossFunc(self, y_true, t_true, pred_t, pred_y_c, pred_y_a, pred_y_a_c):
        ...
        loss_a = self.log_loss(pred_y_a, y_true, False) + self.ipm(self.x_a, bw=False)
        loss_c = self.log_loss(pred_y_c, y_true, False) + self.log_loss(pred_t, t_true, False) + self.ipm(self.x_c, bw=True)
        loss_y_a_c = self.log_loss(pred_y_a_c, y_true, True)
