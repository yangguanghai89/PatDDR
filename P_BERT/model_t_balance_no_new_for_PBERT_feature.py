...
class net(nn.Module):
    def __init__(self, bert_model, tokenizer, p_ipm):
        ...
        self.fc0 = nn.Linear(cfg.dim, cfg.dim)
        self.fc1 = nn.Linear(cfg.dim, cfg.dim)
        self.activ = nn.Tanh()
        self.drop = nn.Dropout(cfg.p_drop_hidden)
        self.fc2_1 = nn.Linear(cfg.dim * 2, cfg.dim)
        self.fc2_2 = nn.Linear(cfg.dim * 2, cfg.dim)
        self.fc3 = nn.Linear(cfg.dim * 2, cfg.dim)
    ...
    def forward(self, state, labels, text_a, text_b, sClass, section, bClass, index, inv_flag, app_flag, tr): #text_a, text_b, sClass, index, section, bClass, inv_flag, app_flag, True
        ...
        for feature in features:
            input_ids, segment_ids, soft_position, token_count, input_mask = feature[0], feature[1], feature[2], \
            feature[3], feature[4]
        ...
        h0_res = h[0][:, 0]
        pooled_h0 = self.drop(self.activ(self.fc0(h[0][:, 0])))  # only use the first h in the sequence
        pooled_h1 = self.drop(self.activ(self.fc1(h[1][:, 0])))  # only use the first h in the sequence
        pooled_h2 = torch.abs(pooled_h0 - pooled_h1)

        pooled_h0 = torch.cat([pooled_h0, pooled_h2], 1)
        pooled_h1 = torch.cat([pooled_h1, pooled_h2], 1)
        pooled_h0 = self.drop(self.activ(self.fc2_1(pooled_h0)))  # only use the first h in the sequence
        pooled_h1 = self.drop(self.activ(self.fc2_2(pooled_h1)))  # only use the first h in the sequence
        pooled_h = torch.cat([pooled_h0, pooled_h1], 1)

        pooled_h = self.drop(self.activ(self.fc3(pooled_h)))
        self.text_embedding = torch.cat([h0_res, pooled_h], 1)
    ...
    def handle_data(self, state, text_a, text_b):
        ...
        for ta, tb in zip(text_a, text_b):
            # 数据增强
            instances = self.dataAugmentation2(('0', ta, tb), ratio=2)
            num = len(instances)
            for i in range(num):
                if i % 2 == 0:
        ...
        features = []
        d0_tensor = ([torch.tensor(x, dtype=torch.long) for x in zip(*data0)])
        d1_tensor = ([torch.tensor(x, dtype=torch.long) for x in zip(*data1)])
        features.append(d0_tensor)
        features.append(d1_tensor)
