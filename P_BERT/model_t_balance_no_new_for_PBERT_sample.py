...
import train, tokenization, checkpoint
from classify_pat_src import Tokenizing_with_position_count_for_prefixSPE, AddSpecialTokensWithTruncation2, TokenIndexing2
...
class net(nn.Module):
    def __init__(self, bert_model, tokenizer, p_ipm):
        ...
        pre_train_mode_file = '/home/wangfei/study/dataset/wangfei/model/uncased_L-12_H-768_A-12/bert_model.ckpt'
        self.load(pre_train_mode_file)
    ...
    def forward(self, state, labels, text_a, text_b, sClass, section, bClass, index, inv_flag, app_flag, tr): #text_a, text_b, sClass, index, section, bClass, inv_flag, app_flag, True
        ...
        if state == 1:    # 训练状态
            self.t = (aug_matrix * self.t).t().flatten()
            self.index = (aug_matrix * self.index).t().flatten()
            self.inv_flag = (aug_matrix * self.inv_flag).t().flatten()
            self.app_flag = (aug_matrix * self.app_flag).t().flatten()
            self.labels = (aug_matrix * self.labels).t().flatten()
        ...
        outputs = self.model(state, input_ids, segment_ids, soft_position, token_count, input_mask)
        self.text_embedding = outputs[:, 0]

        # 单独评估 P_BERT_SAM
        self.pred_y = self.y_text(self.text_embedding)
        #self.pred_y = self.softmax(self.pred_y)[:, 1].unsqueeze(-1)
    ...
    def lossFunc(self):  #, pred_t_c, pred_y_c, pred_t_a, pred_y_a, pred_y_a_c):
        ...
        # 单独评估P_BERT_SAM
        ''''''
        loss = self.criterion(self.pred_y, self.labels.to(torch.int64))
        #loss = self.log_loss(self.pred_y, self.labels, False)
        return loss
    ...
    def load(self, pretrain_file):
        print('Loading the pretrained model from', pretrain_file)
        if pretrain_file.endswith('.ckpt'): # checkpoint file in tensorflow
            checkpoint.load_model(self.model, pretrain_file)
