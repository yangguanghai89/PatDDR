...
from model_t_balance import model_t_balance

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
class mheads_model(nn.Module):
    def __init__(self, bert_model, tokenizer, p_ipm):
        super(mheads_model,self).__init__()
        self.p_ipm = p_ipm
        self.bert_model = bert_model
        self.tokenizer = tokenizer
        self.softmax = nn.Softmax(dim = 1)

        self.text_inv_app_ipc = model_t_balance(self.p_ipm)   # text+inv+app --> ipc
        self.text_ipc_inv_app = model_t_balance(self.p_ipm)   # text+ipc+inv --> app
        self.text_app_ipc_inv = model_t_balance(self.p_ipm)   # text+app+ipc --> inv
    ...
    def forward(self, text_a, text_b, sClass, section, bClass, index, inv_flag, app_flag, tr): #text_a, text_b, sClass, index, section, bClass, inv_flag, app_flag, True
        self.tr = tr
        self.t = sClass
        self.index = index
        text_embedding = []
    ...
    def lossFunc(self, y_true):  #, pred_t_c, pred_y_c, pred_t_a, pred_y_a, pred_y_a_c):
